...
def assemble_commands(samples, idx_prefix, output_path, n_cores=1):
    ...
    The mapping is done using the following commands
        For paired reads:
            hisat2 -p 8 --dta -x <index folder with prefix> -1 <pair1> -2 <pair2> -S <outputfile.sam>
        For unpaired reads:
            hisat2 -p 8 --dta -x <index folder with prefix> -U <unpaired> -S <outputfile.sam>
    """
    ...
    for s_name,sample in samples.items():
        ...
        for p1, p2 in pairs:
            ...
            yield f'hisat2 -p {n_cores} --dta -x {idx_prefix} -1 {p1} -2 {p2} -S {S}'
        ...
        yield f'hisat2 -p {n_cores} --dta -x {idx_prefix} -U {U} -S {S}'
# ---
...
# Set the paralell environment with shared memory
# Specify numbers of core to use
#$ -pe openmp {CORES}
...
#   -> The output file is the next string after the '-S' argument
output_file = command.split()[command.split().index('-S') + 1]

# Execute the job
print("Task ", $SGE_TASK_ID, end='. ')

if Path(output_file).exists():
    print('Output file already exists:', output_file)
    
else:
    print(f"Executing command @", maya.now(), ": ", command)

    @(command.split()) # <-- Here it is executed, splitting is 
                       #     necessary to pass the arguments appropriately
                       
    print("Finished execution @", maya.now())
"""

script_name = 'script.autogenerated.rnaseq_map_jobs.xsh'
