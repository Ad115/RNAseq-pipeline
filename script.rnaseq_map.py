...
bash_exec_path = get_output('which bash')

# To make an array with bash is tricky, we are here generating 
# lines of the form: COMMAND_LIST[1]="XXX -X XXX XXX" (the Xs stand for the actual command)
commands_bash_list = '\n'.join(
        [f'COMMAND_LIST[{i}]="{s}"' for i,s in enumerate(commands)]
    )

# We also need the output filenames.
output_filenames_list = '\n'.join(
        [f'OUTFILE_LIST[{i}]={command.split()[command.split().index("-S") + 1]}' 
...
script_name = 'script.autogenerated.rnaseq_map_jobs.sh'
